...
import com.feather.game.player.CasketRewards;
	...
	public static void handleItemOption1(Player player, final int slotId,
		...
		for (int i: ClueScrolls.ScrollIds){
			if (itemId == i){
				...
				if (itemId == 405) {
					CasketRewards.giveReward(player);
				}
		...
		if (Magic.useTabTeleport(player, itemId))
			return;
		if (Magic.useAltarTeleport(player, itemId))
			return;
	...
	public static void handleItemOnItem(final Player player, InputStream stream) {
		...
		if (interfaceId == Inventory.INVENTORY_INTERFACE
			...
			if (Firemaking.isFiremaking(player, itemUsed, usedWith))
				return;
			//Makes Stackable Old Coins
			if (itemUsed.getId() == 6964 && usedWith.getId() == 6964) {
				if (player.getInventory().containsItem(6964, 3)) {
					player.getInventory().addItem(29997, 1);
					player.getInventory().deleteItem(6964, 3);
				} else {
					player.getPackets().sendGameMessage("You don't have the correct amount to combine the coins.");
				}
			}
